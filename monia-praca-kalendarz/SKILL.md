...
1. Note the attached screenshot's file path on disk.
2. Dispatch **three independent fresh subagents** (Agent tool, default/general-purpose type, run in parallel — a single message with three tool calls), each given this exact prompt with the real path substituted in:

   > Read the image at `<path>`. It's a monthly work-shift calendar screenshot in Polish. Extract every day cell into a JSON array of `{date, status, start, end}`, where `date` is the real absolute `YYYY-MM-DD` (attribute grayed-out overflow cells at the grid's edges to their true adjacent month, not the header month — a grayed cell with a *larger* day number than the first non-grayed cell belongs to the previous month; a grayed cell with a *smaller* day number than the last non-grayed cell belongs to the next month), `status` is one of `work`/`off`/`vacation`/`unclear`, and `start`/`end` (as `H:MM` strings, only present when status is `work`) come from the `od HH:MM do HH:MM` text. If a single cell shows more than one `od–do` range (a split shift), set `status` to `unclear` for that date instead of picking or merging a range. Return only the JSON array as your final message, nothing else.
